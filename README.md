...
VOCA 是一款原生的 macOS 選單列聽寫與翻譯工具。金鑰你自己帶，模型你自己挑，
...
- **金鑰自己帶。** 支援 Groq、OpenAI、Anthropic、Deepgram，想完全離線就用
  Apple Speech。最省的組合一次聽寫不到一分錢。
- **幫你把話說好。** LLM 會去掉口頭禪和贅字，處理你講到一半改口的地方，看到
  問候和署名就排成 email，口頭列的點也會整理成編號清單。它只做整理，不會自己
  加內容。
- **字典會自己長大。** 聽寫完你順手改掉的錯字，VOCA 會默默記起來，下次就不會
  再錯。
- **中文是一等公民。** 繁簡分得清楚，中英夾雜原樣保留，中英文和數字之間也會
  自動補上半形空格，讓「用VOCA」變成「用 VOCA」。
- **邊說邊翻。** 用一種語言講，貼出來是另一種語言。
- **介面乾淨、原生。** 就是個正常的選單列 app，沒有毛玻璃、沒有光暈，也沒有
  那種濃濃的 AI 味，只有溫潤的米白配上 SF Pro。
