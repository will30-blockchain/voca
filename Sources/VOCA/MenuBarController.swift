...
final class MenuBarController {
    ...
    private func configureButton() {
        ...
        button.toolTip = "VOCA — Right Option to dictate · Right Option + Right Shift to translate"
    }
    ...
    private func symbol(for state: EngineState) -> NSImage? {
        ...
        let img = NSImage(systemSymbolName: name, accessibilityDescription: "VOCA") ?? NSImage()
        img.isTemplate = true
        return img
    }
    ...
    private func rebuildMenu(state: EngineState) {
        ...
        let about = NSMenuItem(title: "About VOCA", action: #selector(handleAbout), keyEquivalent: "")
        about.target = self
        menu.addItem(about)

        let quit = NSMenuItem(title: "Quit VOCA", action: #selector(handleQuit), keyEquivalent: "q")
        quit.target = self
        menu.addItem(quit)
